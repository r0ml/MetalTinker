...
#define shaderName Circle_emitting
...
fragmentFn() {
  ...
  float r = mod(uni.iTime, 2.5);
  
  float4 circleColour = float4(snoise(float2(r, r)), 0.5*snoise(uv*0.5), 0.5*snoise(uv*0.5), r);
  float4 backgroundColour = float4(0.0, 0.0, 0.0, 1.0);
  
  float dist = sqrt(dot(uv, uv));
  
  float t = 1.0 + smoothstep(r, r+0.1, dist) - smoothstep(r-0.1, r, dist);
  return mix(circleColour, backgroundColour, t);
}
