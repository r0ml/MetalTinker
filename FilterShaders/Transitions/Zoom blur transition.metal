...
#define shaderName zoom_blur_transition
...
constant const float strengthx = 0.3;
...
float3 crossFade( float2 uv, float dissolve, texture2d<float> tex0, texture2d<float> tex1) {
...
fragmentFn(texture2d<float> tex0, texture2d<float> tex1) {
  ...
  float strength = Sinusoidal_easeInOut(0.0, strengthx, 0.5, progress);
