...
namespace shaderName {
  ...
  float3 overlay(const float3 s, const float3 d ) {
    float3 l = 2 * s * d;
  ...
  float3 levels(const float3 col, const float3 inleft, const float3 inright, const float3 outleft, const float3 outright) {
    float3 res = clamp(col, inleft, inright);
    res = (res - inleft) / (inright - inleft);
    return outleft + res * (outright - outleft);
  }

  float3 contrastAdjust( const float3 color, const float c) {
