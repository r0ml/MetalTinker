...
fragmentFunc(texture2d<float> tex) {
  ...
  samplePosition.x = samplePosition.x+(rand(float2(t,thisVertex.where.y))-0.5)/64.0;
  ...
  texColor = texColor + (float4(-0.5)+float4(rand(float2(thisVertex.where.y,t)),rand(float2(thisVertex.where.y,t+1.0)),rand(float2(thisVertex.where.y,t+2.0)),0))*0.1;
  
  // Either sample the texture, or just make the pixel white (to get the staticy-bit at the bottom)
  whiteNoise = rand(float2(floor(samplePosition.y*80.0),floor(samplePosition.x*50.0))+float2(t,0));
