...
float4 getCol2(float2 pos, texture2d<float> vid0)
{
  float2 uv=pos/textureSize(vid0);
  float4 c1 = vid0.sample(iChannel0,uv);
...
float2 getGrad(float2 pos,float delta, texture2d<float> vid0)
{
...
float2 getGrad2(float2 pos,float delta, texture2d<float> vid0)
{
  float2 d=float2(delta,0);
  return float2(
