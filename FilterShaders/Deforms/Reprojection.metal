...
static float3 RotateY( const float3 vPos, const float s, const float c )
{
...
static float GetDistanceBox(const float3 vPos, const float3 vDimension)
{
  return length(max(abs(vPos)-vDimension,0.0));
}

// result is x=scene distance y=material or object id; zw are material specific parameters (maybe uv co-ordinates)
static float GetDistanceScene( const float3 vPos )
{   
...
static float Raymarch( const C_Ray ray )
{        
  float fDistance = 0.1;
...
static float3 GetColor( C_Ray ray, texture2d<float> tex0, const bool shadow )
{
...
fragmentFunc(texture2d<float> tex, device InputBuffer& in)
{
  ...
  float3 vResult = GetColor( ray , tex, in.ENABLE_SHADOW);
