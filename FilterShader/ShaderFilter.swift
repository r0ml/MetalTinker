...
stat ?
  ...
  /*
    ...
    */
        ...
        makeEncoder(commandBuffer, scale, delegate: delegate)
...
//    var label: String
    ...
    func makeRenderTextures(_ canvasSize : CGSize) {
        if let ri = metadata.fragmentArguments?.first(where: {$0.name == "renderInput"}) {
          ...
          for _ in 0..<mc {
            if let z = makeRenderPassTexture(myName, size: canvasSize) {
  ...
  static func setupRenderPipeline(vertexFunction: MTLFunction?, fragmentFunction: MTLFunction?) -> (MTLRenderPipelineState, MTLRenderPipelineReflection)? {
    // ============================================
...
/*  static func makeDepthAttachmentDescriptor(size canvasSize : CGSize) -> MTLRenderPassDepthAttachmentDescriptor {
  ...
  */
  ...
  func justInitialization() async {
    ...
    guard let initializationProgram = await Self.function.find( nam ) else {
...
//        computeEncoder.setBuffer(uniformBuffer, offset: 0, index: uniformId)
  ...
  /*
    func setupPipelines() async {
      ...
      if let vertexProgram = await Self.function.find("flatVertexFn"),
         let fragmentProgram = await currentFragmentFn(myName),
